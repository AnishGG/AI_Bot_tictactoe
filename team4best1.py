...
##			print "colarr[",i,"] ", colarr[i]
			...
			if (colarr[i] < 0):
				pass
			if (rowarr[i] == 1):
				block_goodness += (self.heur_no)
			if (rowarr[i] == 2):
				block_goodness += (self.heur_no**2)
			if (rowarr[i] == 3):
				block_goodness += (self.heur_no**3)
			if (rowarr[i] == 4):
				block_goodness += 100
			if (colarr[i] == 1):
				block_goodness += (self.heur_no)
	...
	def cal_diam_weight_t(self, centre_x, centre_y, board_status, flag, unflag):
		...
		diam_weight = 0
		if (counter == 1):
			diam_weight += self.heur_no
		if (counter == 2):
			diam_weight += (self.heur_no**2)
		if (counter == 3):
			diam_weight += (self.heur_no**3)
		if (counter == 4):
			diam_weight += 100
